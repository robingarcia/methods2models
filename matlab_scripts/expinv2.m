function [x,xlo,xup] = expinv2(p,mu,pcov,alpha)
...
%
%   [X,XLO,XUP] = EXPINV2(P,MU,PCOV,ALPHA) produces confidence bounds
...
if all(k(:))
    ...
    q = zeros(size(p),'like',internal.stats.dominantType(p,mu)); % single if p or mu is
    q(k) = -log(1-p(k));
